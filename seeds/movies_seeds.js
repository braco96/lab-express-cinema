// Script encargado de rellenar la base de datos con películas de ejemplo
// Cargamos variables de entorno y conexión a la base de datos
require('dotenv/config');
const mongoose = require('mongoose'); // Necesario para cerrar la conexión
require('../db'); // Conexión a MongoDB usando la configuración del proyecto

// Importamos el modelo Movie para poder crear documentos
const Movie = require('../models/Movie.model');

// Array con la información de las películas que insertaremos
const movies = [
  {
...
// Insertamos las películas y cerramos la conexión
Movie.insertMany(movies)
  .then(inserted => {
    console.log(`Se han insertado ${inserted.length} películas`); // Informamos cuántas películas se añadieron
    return mongoose.connection.close(); // Cerramos la conexión con la base de datos
  })
  .catch(err => {
    console.error('Error insertando películas:', err);
    mongoose.connection.close(); // Cerramos también en caso de error
  });
